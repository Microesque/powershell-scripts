<#
.SYNOPSIS
Updates the cash points of an account via the MSSQL database.

.DESCRIPTION
Connects to the MSSQL database and updates the cash point value of the specified
...
The name of the account whose cash points will be updated.

.PARAMETER Value
The cash points value to set. Prefix the value with `+` or `-` to add or
...
- Cash points cannot be set below `0`. If subtraction results in a negative
